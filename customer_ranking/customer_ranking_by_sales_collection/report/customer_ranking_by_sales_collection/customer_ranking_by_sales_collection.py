...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		...
		"customer": ["customer", "base_amount","paid_amount","outstanding_amount","sales_collection_ratio","outstanding_collection_ratio","sales_person"],
	})
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"outstanding_collection_ratio": _("Outstanding Collection Ratio")+ ":Percent:180",
...
class CustomerCollectionByRankingGenerator(object):
	...
	def process(self):
		...
		for row in self.si_list:
			...
			row.paid_amount = flt(row.base_amount - row.outstanding_amount, self.currency_precision)
			row.outstanding_amount = flt(row.outstanding_amount, self.currency_precision)
			...
			row.sales_collection_ratio=flt((row.paid_amount/ row.base_amount) * 100.0,self.currency_precision)
			
			# get outstanding collection ratio
			if row.outstanding_amount:
				row.outstanding_collection_ratio=flt((row.paid_amount/ row.outstanding_amount) * 100.0,self.currency_precision)
			else:
			...
			if self.filters.get("group_by") != "Invoice":
				for i, row in enumerate(self.grouped[key]):
					if i==0:
						...
						new_row.outstanding_amount += flt(row.outstanding_amount, self.currency_precision)
						new_row.paid_amount += flt(row.base_amount-row.outstanding_amount, self.currency_precision)
	...
	def set_average_rate(self, new_row):
		...
		new_row.sales_collection_ratio=flt(((new_row.paid_amount/ new_row.base_amount) * 100.0),self.currency_precision)
		new_row.outstanding_collection_ratio=flt(((new_row.paid_amount/ new_row.outstanding_amount) * 100.0),self.currency_precision) if new_row.paid_amount else 0
